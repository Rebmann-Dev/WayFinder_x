...
class LocalToolAgent:
    ...
    _DESTINATION_KNOWLEDGE_RE = re.compile(
        r"(?:surf|surfing|hike|hiking|trek|trail|food|eat|restaurant|dish|"
        r"wildlife|animals|birds|national park|reserve|nature|budget|cost|"
        r"price|cheap|expensive|lodging|hotel|hostel|weather|climate|rain|"
        r"season|culture|etiquette|customs|transport|bus|taxi|airport|"
        r"vaccine|health|medical|visa|entry|border)",
        re.IGNORECASE,
    )
    ...
    def _check_country_json(self, query: str) -> str | None:
        ...
        country_map = {"Ecuador": "ec", "Peru": "pe"}
        ...
        if not country_code:
            ...
            if any(c in dest_city for c in ["quito", "guayaquil", "cuenca", "manta"]):
                country_code = "ec"
            elif any(c in dest_city for c in ["lima", "cusco", "arequipa", "mancora"]):
                country_code = "pe"

        if not country_code:
            # Also try selected_location
            selected = st.session_state.get("selected_location") or {}
            sel_country = selected.get("country", "")
            ...
            if not country_code:
                ...
                if any(c in sel_city for c in ["quito", "guayaquil", "cuenca", "manta"]):
                    country_code = "ec"
                elif any(c in sel_city for c in ["lima", "cusco", "arequipa", "mancora"]):
                    country_code = "pe"
        ...
        if result:
            # Format the cached data into a readable answer
        ...
        if not user_wants_flights and not user_wants_safety:
            ...
            if latest_q and self._DESTINATION_KNOWLEDGE_RE.search(latest_q):
                ...
                if json_answer:
                    ...
                    log.info("AGENT DONE at JSON-first lookup")
                    return
                else:
