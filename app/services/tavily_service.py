...
# ── Central America override (pycountry_convert puts these under NA) ──────────
_CENTRAL_AMERICA_CODES = {"cr", "pa", "gt", "bz", "hn", "ni", "sv"}
...
def _continent_for_new_country(cc: str) -> str:
    """Determine continent subfolder for a country code we don't have a JSON for yet.
    Uses pycountry_convert if available, otherwise falls back to 'unknown'.
    Central American countries are overridden from NA → central_america.
    """
    cc = cc.strip().lower()

    if cc in _CENTRAL_AMERICA_CODES:
    ...
    try:
        ...
        continent_code = pc.country_alpha2_to_continent_code(cc.upper())
        return _CONTINENT_CODE_MAP.get(continent_code, "unknown")
    except Exception:
        return "unknown"


def _find_country_json(country_code: str) -> Path | None:
    """Locate a country JSON file by code or name, scanning all continent
    subfolders dynamically. Normalizes underscores and spaces so that
    'costa_rica', 'costa rica', and 'costarica' all resolve correctly.
    """
    if not country_code:
        return None
    cc = country_code.strip().lower()
    cc_normalized = cc.replace("_", "").replace(" ", "")

    # Scan ALL continent subfolders dynamically — no hardcoded list needed
    for continent_dir in sorted(_COUNTRIES_DIR.iterdir()):
        if not continent_dir.is_dir() or continent_dir.name.startswith("."):
            continue

        # 1. Exact filename match (e.g. "cr.json" or "costa_rica.json")
        if (continent_dir / f"{cc}.json").exists():
            return continent_dir / f"{cc}.json"

        # 2. Normalized match — strips _ and spaces from both sides
        for p in continent_dir.glob("*.json"):
            stem_normalized = p.stem.lower().replace("_", "").replace(" ", "")
            if stem_normalized == cc_normalized:
                return p

        # 3. Prefix match as last resort (e.g. "costa" → "costarica")
        for p in continent_dir.glob("*.json"):
            stem_normalized = p.stem.lower().replace("_", "").replace(" ", "")
            if stem_normalized.startswith(cc_normalized) and len(cc_normalized) >= 4:
                return p

    # Root-level fallback (for any JSONs sitting directly in countries/)
    if (_COUNTRIES_DIR / f"{cc}.json").exists():
        return _COUNTRIES_DIR / f"{cc}.json"
...
class TavilyService:
    ...
    def _create_country_json(country_code: str) -> Path | None:
        ...
        cc = country_code.strip().lower()
        continent = _continent_for_new_country(cc)
...
_CONTINENT_FOLDERS = [
    ...
    "europe", "asia", "southeast_asia", "africa", "oceania"
]
